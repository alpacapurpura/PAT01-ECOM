...
class Global_Hooks {
	...
	public function __construct() {
		...
		// Display user defined fields in order edit page.
		add_action( 'woocommerce_admin_order_data_after_billing_address', [ $this, 'add_item_to_order_details' ], 10, 1 );
		add_action( 'woocommerce_admin_order_data_after_shipping_address', [ $this, 'add_item_to_order_details' ], 10, 1 );
		...
		$action            = isset( $_REQUEST['action'] ) ? filter_var( $_REQUEST['action'], FILTER_SANITIZE_FULL_SPECIAL_CHARS ) : ''; // phpcs:ignore
		$elementor_preview = isset( $_REQUEST['elementor-preview'] ) ? filter_var( $_REQUEST['elementor-preview'], FILTER_SANITIZE_FULL_SPECIAL_CHARS ) : ''; // phpcs:ignore

		if (
			( 'elementor' === $action && is_admin() ) // Elementor Editor.
			|| 'elementor_ajax' === $action // Elementor Editor Preview - Ajax Render Widget.
			|| ! empty( $elementor_preview ) // Elementor Editor Preview.
		) {
	...
	/**
	 * Add new item to the WooCommerce order details on the edit page.
	 *
	 * @param object $order woocommerce order object.
	 * @since 1.8.9
	 */
	public function add_item_to_order_details( $order ) {
		$order_id       = $order->get_id();
		$custom_fields  = get_post_meta( $order_id, 'sellkit_checkout_widget_custom_field_of_order', true );
		...
		foreach ( $custom_fields as $field ) {
			...
			if (
				'woocommerce_admin_order_data_after_shipping_address' === $current_action &&
				'shipping' === substr( $key_name, 0, 8 )
			) {
				?>
					<h3><?php echo esc_html( $label ); ?></h3>
					<div><?php echo esc_html( get_post_meta( $order_id, $key_name, true ) ); ?></div>
				<?php
			}

			if (
				'woocommerce_admin_order_data_after_billing_address' === $current_action &&
				'billing' === substr( $key_name, 0, 7 )
			) {
				?>
					<h3><?php echo esc_html( $label ); ?></h3>
					<div><?php echo esc_html( get_post_meta( $order_id, $key_name, true ) ); ?></div>
