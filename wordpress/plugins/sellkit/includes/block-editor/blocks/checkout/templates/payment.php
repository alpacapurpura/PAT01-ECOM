...
if ( ! wp_doing_ajax() ) {
	?>
		...
		<p class="sellkit-one-page-checkout-payment-desc sub-heading">
			<?php
				$sk_transaction_text = apply_filters( 'sellkit_checkout_block_secure_transaction_text', esc_html__( 'All transactions are secure and encrypted.', 'sellkit' ) );
				echo wp_kses_post( $sk_transaction_text );
			?>
		</p>
	<?php
}
